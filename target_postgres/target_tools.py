...
def stream_to_target(stream, target, config={}):
    ...
    try:
        ...
        batch_detection_threshold = config.get(
            "batch_detection_threshold", max(max_batch_rows / 40, 50)
        )
...
def _report_invalid_records(streams):
    for stream_buffer in streams.values():
        if stream_buffer.peek_invalid_records():
            LOGGER.warning(
                "Invalid records detected for stream {}: {}".format(
                    stream_buffer.stream, stream_buffer.peek_invalid_records()
                )
            )
...
def _line_handler(
    ...
    if line_data["type"] == "SCHEMA":
        ...
        if line_data["stream"] not in state_tracker.streams:
            raise TargetError(
                "A ACTIVATE_VERSION for stream {} was encountered before a corresponding schema".format(
                    line_data["stream"]
                )
            )
        ...
        raise TargetError(
            "Unknown message type {} in message {}".format(line_data["type"], line)
        )
...
def _send_usage_stats():
    try:
        ...
        with http.client.HTTPConnection(
            "collector.singer.io", timeout=10
        ).connect() as conn:
            params = {
                "e": "se",
                "aid": "singer",
